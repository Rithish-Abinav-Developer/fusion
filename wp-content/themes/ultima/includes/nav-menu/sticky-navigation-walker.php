 ...
 */
if ( ! class_exists( 'UltimaQodefStickyNavigationWalker' ) ) {
	class UltimaQodefStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$flip_animation = ultima_qodef_options()->getOptionValue( 'enable_animate_main_menu_items' ) == "yes";
			...
			//menu type class
			$wide_background = '';
			if ( ultima_qodef_options()->getOptionValue( 'enable_wide_menu_background' ) == "yes" ) {
			...
			if ( $item->hide == "" ) {
				...
				//Open flip holder
				if ( $depth == 0 ) {
					$item_output .= "<span class='qodef-menu-flip-holder'>";
				}
				...
				$item_structure .= '<span class="item_outer">';
				...
				$item_structure .= '<span class="item_inner">';
				...
				$item_structure .= '<span class="menu_icon_wrapper"><i class="menu_icon ' . $icon . '"></i></span>';
				$item_structure .= '<span class="item_text">';
				$item_structure .= apply_filters( 'the_title', $item->title, $item->ID );
				$item_structure .= '</span>'; //close span.item_text
				if ( $item->featured_icon !== "" && $item->featured_icon == "fa-star" ) {
					$item_structure .= '<span class=" qodef-featured-icon fa ' . $item->featured_icon . '" aria-hidden="true"></span>';
				} else if ( $item->featured_icon !== "" && $item->featured_icon == "New" ) {
					$item_structure .= '<span class="qodef-featured-icon text" aria-hidden="true">' . esc_html__( "New", "ultima" ) . '</span>';
				}
				$item_structure .= '</span>'; //close span.item_inner
				$item_structure .= '<span class="plus"></span>';

				//append arrow for dropdown

				if ( $args->has_children && $angle_icon != "" ) {
					$item_structure .= '<i class="qodef-menu-arrow fa ' . $angle_icon . '"></i>';
				}

				$item_structure .= '</span>'; //close span.item_outer

				if ( $flip_animation && $depth == 0 ) {
					$item_output .= "<span class='qodef-flip-first'>";
				}

				$item_output .= $item_structure; //this is buffered html that needs to be rendered anyway

				if ( $flip_animation && $depth == 0 ) {
					$item_output .= "</span>";
				}
				if ( $flip_animation && $depth == 0 ) {
					$item_output .= "<span class='qodef-flip-second'>";
					$item_output .= $item_structure;
					$item_output .= "</span>";
				}

				$item_output .= "</a>"; //closing link
