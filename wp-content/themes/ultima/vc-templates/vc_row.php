...
if ( ! empty( $row_type ) && $row_type == 'parallax' ) {
	...
	if ( ultima_qodef_options()->getOptionValue( 'parallax_on_off' ) == 'off' ) {
...
if ( $row_type == 'row' && $css_animation != '' ) {
	...
	if ( $transition_delay !== '' ) {
		...
		$inner_start        .= '<div ' . ultima_qodef_get_inline_style( $animation_styles ) . '>';
		$inner_end          .= '</div>';
	} else {
		$inner_start .= '<div>';
		$inner_end   .= '</div>';
...
if ( $angled_shape == 'yes' && $angled_shape_top == 'yes' ) {
	$svg_top_output .= '<svg class="qodef_angled_shape qodef_svg_top" preserveAspectRatio="none" viewBox="0 0 86 86" width="100%" height="570">';
...
if ( $angled_shape == 'yes' && $angled_shape_bottom == 'yes' ) {
	$svg_bottom_output .= '<svg class="qodef_angled_shape qodef_svg_bottom" preserveAspectRatio="none" viewBox="0 0 86 86" width="100%" height="570">';
...
if ( $arrow_down == 'yes' ) {
	...
	$arrow_icon .= '<a class="qodef-anchor" itemprop="url" href="#' . $arrow_down_anchor . '" ' . ultima_qodef_get_inline_style( implode( ';', $arrow_icon_style ) ) . '>';
	$arrow_icon .= '<span class="arrow_carrot-down"></span>';
	$arrow_icon .= '</a>';
	$arrow_icon .= '</div>';
...
if ( $video == 'show_video' ) {
	...
	if ( $video_image ) {
		$video_output .= '<div class="qodef-mobile-video-image" ' . ultima_qodef_get_inline_attr( $video_mobile_style, 'style' ) . '></div>';
	}
	$video_output .= '<div ' . ultima_qodef_get_class_attribute( $video_overlay_class ) . ultima_qodef_get_inline_attr( $video_overlay_style, 'style' ) . '></div>';
	$video_output .= '<div class="qodef-video-wrap">';
	$video_output .= '<video class="qodef-video" width="1920" height="800" ' . ultima_qodef_get_inline_attr( $video_attrs, 'poster' ) . ' controls="controls" preload="auto" loop autoplay muted>';
	if ( ! empty( $video_webm ) ) {
	...
	if ( $v_image ) {
		$video_output .= '<img ' . ultima_qodef_get_inline_attr( $v_image, 'src' ) . ' width="1920" height="800" title="' . esc_attr_e( 'No video playback capabilities', 'ultima' ) . '" alt="' . esc_attr__( 'Video Thumb', 'ultima' ) . '" />';
	}
	$video_output .= '</object>';
	$video_output .= '</video>';
	$video_output .= '</div>';
...
print ultima_qodef_get_module_part( $output );