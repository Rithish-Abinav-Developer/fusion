...
class UltimaQodefFramework {
	...
	private function __construct() {
		$this->qodeOptions   = UltimaQodefOptions::get_instance();
		$this->qodeMetaBoxes = UltimaQodefMetaBoxes::get_instance();
	}
	...
	public function setSkin( UltimaQodefSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class UltimaQodefSkinManager
 *
	...
	private function setSkin( $skinName = 'select' ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$this->skin = new UltimaSelectSkin();
			}
		} else {
			$this->skin = false;
		}
...
/**
 * Class UltimaQodefSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class UltimaQodefSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( ultima_qodef_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
        <script>
            window.qodeSCIcon = '<?php echo ultima_qodef_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
class UltimaQodefOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $ultima_qodef_options ) ) {
			return $ultima_qodef_options[ $key ];
...
/*
   Class: UltimaQodefAdminPage
   A class that initializes Qode Admin Page
*/

class UltimaQodefAdminPage implements iUltimaQodefLayoutNode {
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new UltimaQodefFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iUltimaQodefRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: UltimaQodefMetaBoxes
...
/*
   Class: UltimaQodefMetaBox
   A class that initializes Qode Meta Box
*/

class UltimaQodefMetaBox implements iUltimaQodefLayoutNode {
	...
	public function setTitle( $label ) {
		...
		return $ultima_qodef_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

if ( ! function_exists( 'ultima_qodef_init_framework_variable' ) ) {

	function ultima_qodef_init_framework_variable() {

		global $ultima_qodef_Framework;

		$ultima_qodef_Framework = UltimaQodefFramework::get_instance();
		$qodeSkinManager        = new UltimaQodefSkinManager();
		$ultima_qodef_Framework->setSkin( $qodeSkinManager->getSkin() );
	}

	add_action( 'ultima_qodef_before_options_map', 'ultima_qodef_init_framework_variable' );
}
?>