...
if(ultima_qodef_options()->getOptionValue('blog_single_navigation') == 'yes'){

    $same_category = false;
    if(ultima_qodef_options()->getOptionValue('blog_navigation_through_same_category') == 'yes') {
        $same_category = true;
    }
			...
			<?php if($prev_post != ""){ ?>
				<div class="qodef-blog-single-prev-holder">
					...
					<?php if(has_post_thumbnail($prev_post->ID)){
						...
						?>
						<div class="qodef-blog-single-prev" style="background-image: url(' <?php echo ultima_qodef_kses_img($prev_post_image[0]); ?> ')">
							<?php
							...
							<h4 itemprop="name" class="qodef-blog-single-nav-title">
								<a itemprop="url" href="<?php echo get_permalink($prev_post->ID)?>">
                                    <span class="qodef-blog-navigation-info">
                                        <?php esc_html_e( 'Previous post', 'ultima' )?>
                                    </span>
			...
			<?php if($next_post != ""){ ?>
				<div class="qodef-blog-single-next-holder">
						...
						<div class="qodef-blog-navigation-info-holder clearfix">
							<h4 itemprop="name" class="qodef-blog-single-nav-title">
								<a itemprop="url" href="<?php echo get_permalink($next_post->ID)?>" class="qodef-blog-single-nav-title">
                                    <span class="qodef-blog-navigation-info">
                                        <?php esc_html_e( 'Next post', 'ultima' )?>
                                    </span>
					...
					<?php if(has_post_thumbnail($next_post->ID)){
						...
						?>
						<div class="qodef-blog-single-next" style="background-image: url(' <?php echo ultima_qodef_kses_img($next_post_image[0]); ?> ')">
							<?php
