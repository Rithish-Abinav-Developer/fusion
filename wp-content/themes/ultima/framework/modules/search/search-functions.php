...
if( !function_exists('ultima_qodef_search_body_class') ) {
	/**
	 ...
	 */
	function ultima_qodef_search_body_class($classes) {
	...
	add_filter('body_class', 'ultima_qodef_search_body_class');
}

if ( ! function_exists('ultima_qodef_get_search') ) {
	/**
	 * Loads search HTML based on search type option.
	 */
	function ultima_qodef_get_search() {

		if ( ultima_qodef_active_widget( false, false, 'qode_search_opener' ) ) {

			ultima_qodef_load_search_template();
...
if ( ! function_exists('ultima_qodef_load_search_template') ) {
	/**
	 * Loads HTML template with parameters
	 */
	function ultima_qodef_load_search_template() {

		$search_type = 'fullscreen-search';

		$search_icon = '';
		$search_icon_close = '';
		if ( ultima_qodef_options()->getOptionValue('search_icon_pack') !== '' ) {
			$search_icon = ultima_qodef_icon_collections()->getSearchIcon( ultima_qodef_options()->getOptionValue('search_icon_pack'), true );
			$search_icon_close = ultima_qodef_icon_collections()->getSearchClose( ultima_qodef_options()->getOptionValue('search_icon_pack'), true );
		}
		...
		ultima_qodef_get_module_template_part( 'templates/types/'.$search_type, 'search', '', $parameters );

	}

}

if ( ! function_exists( 'ultima_qodef_override_search_block_templates' ) ) {
	/**
	 * Function that override `core/search` block template
	 *
	 * @see register_block_core_search()
	 */
	function ultima_qodef_override_search_block_templates( $atts ) {
		if ( ! empty( $atts ) && isset( $atts['render_callback'] ) && 'render_block_core_search' === $atts['render_callback'] && function_exists( 'styles_for_block_core_search' ) ) {
			$atts['render_callback'] = 'ultima_qodef_render_block_core_search';
		}
		
		return $atts;
	}
	
	add_filter( 'block_type_metadata_settings', 'ultima_qodef_override_search_block_templates' );
}

if ( ! function_exists( 'ultima_qodef_render_block_core_search' ) ) {
	/**
	 ...
	 */
	function ultima_qodef_render_block_core_search( $attributes ) {
		static $instance_id = 0;
		
		$attributes = wp_parse_args(
			$attributes,
			array(
				'label'      => esc_html__( 'Search', 'ultima' ),
				'buttonText' => esc_html__( 'Search', 'ultima' ),
			)
		);
		...
		$label_markup = sprintf(
			...
			$input_id,
			empty( $attributes['label'] ) ? esc_html__( 'Search', 'ultima' ) : esc_html( $attributes['label'] )
		);
		...
		if ( $show_button ) {
			...
			if ( ! $use_icon_button ) {
				...
				$button_internal_markup  = ultima_qodef_icon_collections()->renderIcon( 'ion-android-search', 'ion_icons' );
			}
