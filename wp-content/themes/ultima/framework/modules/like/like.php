...
class UltimaQodefLike {
	private static $instance;

	private function __construct() {
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
		add_action( 'wp_ajax_ultima_qodef_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_ultima_qodef_like', array( $this, 'ajax' ) );
	}
	...
	function enqueue_scripts() {
		wp_enqueue_script( 'ultima-qodef-like', QODE_ASSETS_ROOT . '/js/like.js', 'jquery', '1.0', true );
		wp_localize_script( 'ultima-qodef-like', 'qodefLike', array(
	...
	public function like_post( $post_id, $action = 'get', $type = '' ) {
		...
		switch ( $action ) {
			case 'get':
				...
				$return_value = wp_kses_post( $icon ) . "<span>" . esc_attr( $like_count ) . "</span>";
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'ultima' );

		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'ultima' );
		}
...
if ( ! function_exists( 'ultima_qodef_create_like' ) ) {
	function ultima_qodef_create_like() {
		UltimaQodefLike::get_instance();
	}

	add_action( 'after_setup_theme', 'ultima_qodef_create_like' );
}