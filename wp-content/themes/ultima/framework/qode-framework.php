...
if ( ! function_exists( 'ultima_qodef_admin_scripts_init' ) ) {
	/**
	 * Function that registers all scripts that are necessary for our back-end
	 */
	function ultima_qodef_admin_scripts_init() {

		/**
		 * @see QodeSkinAbstract::registerScripts - hooked with 10
		 * @see QodeSkinAbstract::registerStyles - hooked with 10
		 */
		do_action( 'ultima_qodef_admin_scripts_init' );
	}

	add_action( 'admin_init', 'ultima_qodef_admin_scripts_init' );
}

if ( ! function_exists( 'ultima_qodef_enqueue_admin_styles' ) ) {
	/**
	 * Function that enqueues styles for options page
	 */
	function ultima_qodef_enqueue_admin_styles() {
		wp_enqueue_style( 'wp-color-picker' );

		/**
		 * @see QodeSkinAbstract::enqueueStyles - hooked with 10
		 */
		do_action( 'ultima_qodef_enqueue_admin_styles' );
	}
}

if ( ! function_exists( 'ultima_qodef_enqueue_admin_scripts' ) ) {
	/**
	 * Function that enqueues styles for options page
	 */
	function ultima_qodef_enqueue_admin_scripts() {
		...
		wp_enqueue_media();
		wp_enqueue_script( "ultima-qodef-dependence", get_template_directory_uri() . '/framework/admin/assets/js/qodef-ui/qodef-dependence.js', array(), false, true );
		wp_enqueue_script( "ultima-qodef-twitter-connect", get_template_directory_uri() . '/framework/admin/assets/js/qodef-twitter-connect.js', array(), false, true );

		/**
		 * @see QodeSkinAbstract::enqueueScripts - hooked with 10
		 */
		do_action( 'ultima_qodef_enqueue_admin_scripts' );
	}
}

if ( ! function_exists( 'ultima_qodef_enqueue_meta_box_styles' ) ) {
	/**
	 * Function that enqueues styles for meta boxes
	 */
	function ultima_qodef_enqueue_meta_box_styles() {
		wp_enqueue_style( 'wp-color-picker' );

		/**
		 * @see QodeSkinAbstract::enqueueStyles - hooked with 10
		 */
		do_action( 'ultima_qodef_enqueue_meta_box_styles' );
	}
}

if ( ! function_exists( 'ultima_qodef_enqueue_meta_box_scripts' ) ) {
	/**
	 * Function that enqueues scripts for meta boxes
	 */
	function ultima_qodef_enqueue_meta_box_scripts() {
		wp_enqueue_script( 'wp-color-picker' );
		wp_enqueue_script( 'jquery-ui-datepicker' );
		wp_enqueue_script( 'jquery-ui-accordion' );
		wp_enqueue_script( 'common' );
		...
		wp_enqueue_script( 'postbox' );
		wp_enqueue_media();
		wp_enqueue_script( 'ultima-qodef-dependence' );

		/**
		 * @see QodeSkinAbstract::enqueueScripts - hooked with 10
		 */
		do_action( 'ultima_qodef_enqueue_meta_box_scripts' );
	}
}

if ( ! function_exists( 'ultima_qodef_enqueue_nav_menu_script' ) ) {
	/**
	 ...
	 */
	function ultima_qodef_enqueue_nav_menu_script( $hook ) {
		if ( $hook == 'nav-menus.php' ) {
			wp_enqueue_script( 'ultima-qodef-nav-menu', get_template_directory_uri() . '/framework/admin/assets/js/qodef-nav-menu.js' );
			wp_enqueue_style( 'ultima-qodef-nav-menu', get_template_directory_uri() . '/framework/admin/assets/css/qodef-nav-menu.css' );
		}
	}

	add_action( 'admin_enqueue_scripts', 'ultima_qodef_enqueue_nav_menu_script' );
}


if ( ! function_exists( 'ultima_qodef_enqueue_widgets_admin_script' ) ) {
	/**
	 * Function that enqueues styles and scripts for admin widgets page.
	 *
	 * @param $hook string current page hook to check
	 */
	function ultima_qodef_enqueue_widgets_admin_script( $hook ) {
		if ( $hook == 'widgets.php' ) {
			wp_enqueue_script( 'ultima-qodef-dependence' );
		}
	}

	add_action( 'admin_enqueue_scripts', 'ultima_qodef_enqueue_widgets_admin_script' );
}


if ( ! function_exists( 'ultima_qodef_enqueue_styles_slider_taxonomy' ) ) {
	/**
	 * Enqueue styles when on slider taxonomy page in admin
	 */
	function ultima_qodef_enqueue_styles_slider_taxonomy() {
		if ( isset( $_GET['taxonomy'] ) && $_GET['taxonomy'] == 'slides_category' ) {
			ultima_qodef_enqueue_admin_styles();
		}
	}

	add_action( 'admin_print_scripts-edit-tags.php', 'ultima_qodef_enqueue_styles_slider_taxonomy' );
}

if ( ! function_exists( 'ultima_qodef_init_theme_options_array' ) ) {
	/**
	 * Function that merges $ultima_qodef_options and default options array into one array.
	 *
	 * @see array_merge()
	 */
	function ultima_qodef_init_theme_options_array() {
		global $ultima_qodef_options, $ultima_qodef_Framework;

		$db_options = get_option( 'qode_options_ultima' );

		//does qode_options exists in db?
		if ( is_array( $db_options ) ) {
			//merge with default options
			$ultima_qodef_options = array_merge( $ultima_qodef_Framework->qodeOptions->options, get_option( 'qode_options_ultima' ) );
		} else {
			//options don't exists in db, take default ones
			$ultima_qodef_options = $ultima_qodef_Framework->qodeOptions->options;
		}
	}

	add_action( 'ultima_qodef_after_options_map', 'ultima_qodef_init_theme_options_array', 0 );
}

if ( ! function_exists( 'ultima_qodef_init_theme_options' ) ) {
	/**
	 * Function that sets $ultima_qodef_options variable if it does'nt exists
	 */
	function ultima_qodef_init_theme_options() {
		global $ultima_qodef_options;
		global $ultima_qodef_Framework;
		if ( isset( $ultima_qodef_options['reset_to_defaults'] ) ) {
			if ( $ultima_qodef_options['reset_to_defaults'] == 'yes' ) {
				delete_option( "qode_options_ultima" );
			}
		}

		if ( ! get_option( "qode_options_ultima" ) ) {
			add_option( "qode_options_ultima",
				$ultima_qodef_Framework->qodeOptions->options
			);
...
if ( ! function_exists( 'ultima_qodef_register_theme_settings' ) ) {
	/**
	 * Function that registers setting that will be used to store theme options
	 */
	function ultima_qodef_register_theme_settings() {
		register_setting( 'ultima_qodef_theme_menu', 'qode_options' );
	}

	add_action( 'admin_init', 'ultima_qodef_register_theme_settings' );
}

if ( ! function_exists( 'ultima_qodef_get_admin_tab' ) ) {
	/**
	 * Helper function that returns current tab from url.
	 * @return null
	 */
	function ultima_qodef_get_admin_tab() {
		return isset( $_GET['page'] ) ? ultima_qodef_strafter( $_GET['page'], 'tab' ) : null;
	}
}

if ( ! function_exists( 'ultima_qodef_strafter' ) ) {
	/**
	 ...
	 */
	function ultima_qodef_strafter( $string, $substring ) {
...
if ( ! function_exists( 'ultima_qodef_save_options' ) ) {
	/**
	 * Function that saves theme options to db.
	 * It hooks to ajax wp_ajax_qodef_save_options action.
	 */
	function ultima_qodef_save_options() {
		global $ultima_qodef_options;
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
			...
			$ultima_qodef_options = array_merge( $ultima_qodef_options, $_REQUEST );

			update_option( 'qode_options_ultima', $ultima_qodef_options );

			do_action( 'ultima_qodef_after_theme_option_save' );
			echo "Saved";
	...
	add_action( 'wp_ajax_ultima_qodef_save_options', 'ultima_qodef_save_options' );
}

if ( ! function_exists( 'ultima_qodef_meta_box_add' ) ) {
	/**
	 * Function that adds all defined meta boxes.
	 * It loops through array of created meta boxes and adds them
	 */
	function ultima_qodef_meta_box_add() {
		global $ultima_qodef_Framework;

		foreach ( $ultima_qodef_Framework->qodeMetaBoxes->metaBoxes as $key => $box ) {
			$hidden = false;
			if ( ! empty( $box->hidden_property ) ) {
				foreach ( $box->hidden_values as $value ) {
					if ( ultima_qodef_option_get_value( $box->hidden_property ) == $value ) {
						$hidden = true;
					}
				}
			}
			...
			if ( is_array( $box->scope ) && count( $box->scope ) ) {
				foreach ( $box->scope as $screen ) {
					ultima_qodef_create_meta_box_handler( $box, $key, $screen );

					if ( $hidden ) {
						add_filter( 'postbox_classes_' . $screen . '_qodef-meta-box-' . $key, 'ultima_qodef_meta_box_add_hidden_class' );
					}
				}
			}
		}

		if ( ultima_qodef_is_gutenberg_installed() || ultima_qodef_is_gutenberg_editor() ) {
			ultima_qodef_enqueue_meta_box_styles();
			ultima_qodef_enqueue_meta_box_scripts();
		} else {
			add_action( 'admin_enqueue_scripts', 'ultima_qodef_enqueue_meta_box_styles' );
			add_action( 'admin_enqueue_scripts', 'ultima_qodef_enqueue_meta_box_scripts' );
		}
	}
}

if ( ! function_exists( 'ultima_qodef_get_meta_post_types' ) ) {
	function ultima_qodef_get_meta_post_types() {
		$post_types = array(
			"page",
			"post",
			"portfolio-item",
		...
		$post_types = apply_filters( 'ultima_qodef_meta_post_types', $post_types );

		return $post_types;
	}
}

if ( ! function_exists( 'ultima_qodef_meta_box_save' ) ) {
	/**
	 ...
	 */
	function ultima_qodef_meta_box_save( $post_id, $post ) {
		global $ultima_qodef_Framework;
		global $ultima_qodef_IconCollections;

		$nonces_array = array();
		$meta_boxes   = ultima_qodef_framework()->qodeMetaBoxes->getMetaBoxesByScope( $post->post_type );

		if ( is_array( $meta_boxes ) && count( $meta_boxes ) ) {
			foreach ( $meta_boxes as $meta_box ) {
				$nonces_array[] = 'ultima_qodef_meta_box_' . $meta_box->name . '_save';
			}
		}
		...
		$postTypes = ultima_qodef_get_meta_post_types();
		...
		foreach ( $ultima_qodef_Framework->qodeMetaBoxes->options as $key => $box ) {
	...
	add_action( 'save_post', 'ultima_qodef_meta_box_save', 1, 2 );
}

if ( ! function_exists( 'ultima_qodef_render_meta_box' ) ) {
	/**
	 ...
	 */
	function ultima_qodef_render_meta_box( $post, $metabox ) { ?>

        <div class="qodef-meta-box qodef-page">
            <div class="qodef-meta-box-holder">

				<?php $metabox['args']['box']->render(); ?>
				<?php wp_nonce_field( 'ultima_qodef_meta_box_' . $metabox['args']['box']->name . '_save', 'ultima_qodef_meta_box_' . $metabox['args']['box']->name . '_save' ); ?>
...
if ( ! function_exists( 'ultima_qodef_meta_box_add_hidden_class' ) ) {
	/**
	 ...
	 */
	function ultima_qodef_meta_box_add_hidden_class( $classes = array() ) {
...
if ( ! function_exists( 'ultima_qodef_remove_default_custom_fields' ) ) {
	/**
	 * Function that removes default WordPress custom fields interface
	 */
	function ultima_qodef_remove_default_custom_fields() {
	...
	add_action( 'do_meta_boxes', 'ultima_qodef_remove_default_custom_fields' );
}


if ( ! function_exists( 'ultima_qodef_get_custom_sidebars' ) ) {
	/**
	 ...
	 * @uses get_option()
	 */
	function ultima_qodef_get_custom_sidebars() {
...
if ( ! function_exists( 'ultima_qodef_generate_icon_pack_options' ) ) {
	/**
	 * Generates options HTML for each icon in given icon pack
	 * Hooked to wp_ajax_update_admin_nav_icon_options action
	 */
	function ultima_qodef_generate_icon_pack_options() {
		global $ultima_qodef_IconCollections;
        check_ajax_referer('update-nav_menu', 'update_nav_menu_nonce');
		$html               = '';
		$icon_pack          = isset( $_POST['icon_pack'] ) ? $_POST['icon_pack'] : '';
		$collections_object = $ultima_qodef_IconCollections->getIconCollection( $icon_pack );
	...
	add_action( 'wp_ajax_update_admin_nav_icon_options', 'ultima_qodef_generate_icon_pack_options' );
}

if ( ! function_exists( 'ultima_qodef_admin_notice' ) ) {
	/**
	 ...
	 */
	function ultima_qodef_admin_notice( $id, $message, $class, $is_dismisable = true ) {
		...
		if ( ! $is_dismised && is_admin() ) {
			...
			if ( $is_dismisable ) {
				echo '<strong style="display: block; margin-top: 7px;"><a href="' . esc_url( add_query_arg( 'qode_dismis_notice', $id ) ) . '">' . esc_html__( 'Dismiss this notice', 'ultima' ) . '</a></strong>';
			}
...
if ( ! function_exists( 'ultima_qodef_save_dismisable_notice' ) ) {
	/**
	 * Updates user meta with dismisable notice. Hooks to admin_init action
	 * in order to check this on every page request in admin
	 */
	function ultima_qodef_save_dismisable_notice() {
	...
	add_action( 'admin_init', 'ultima_qodef_save_dismisable_notice' );
}

if ( ! function_exists( 'ultima_qodef_hook_twitter_request_ajax' ) ) {
	/**
	 ...
	 */
	function ultima_qodef_hook_twitter_request_ajax() {
        check_ajax_referer( 'qodef_twitter_connect_nonce', 'twitter_connect_nonce' );

		QodefTwitterApi::getInstance()->obtainRequestToken();
	}

	add_action( 'wp_ajax_qode_twitter_obtain_request_token', 'ultima_qodef_hook_twitter_request_ajax' );
}
?>