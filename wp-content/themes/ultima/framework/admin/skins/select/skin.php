...
class UltimaSelectSkin extends UltimaQodefSkinAbstract {
	/**
	 * Skin constructor. Hooks to ultima_qodef_admin_scripts_init and ultima_qodef_enqueue_admin_styles
	 */
	public function __construct() {
		$this->skinName = 'select';

		//hook to admin register scripts
		add_action( 'ultima_qodef_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'ultima_qodef_admin_scripts_init', array( $this, 'registerScripts' ) );

		//hook to admin enqueue scripts
		add_action( 'ultima_qodef_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'ultima_qodef_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );

		add_action( 'ultima_qodef_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'ultima_qodef_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $scriptPath ) {
			ultima_qodef_register_skin_script( $scriptHandle, $scriptPath );
		}
	}
	...
	public function registerStyles() {
		...
		foreach ( $this->styles as $styleHandle => $stylePath ) {
			ultima_qodef_register_skin_style( $styleHandle, $stylePath );
		}
	...
	public function setIcons() {
		...
		$icons_array = apply_filters( 'ultima_qodef_set_skin_icons', $icons_array, $this->getSkinURI() . '/' );
	...
	public function setMenuItemPosition() {
		...
		$positions_array    = apply_filters( 'ultima_qodef_set_skin_menu_positions', $positions_array );
	 ...
	 *
	 * @see UltimaSelectSkin::getHeader()
	 * @see UltimaSelectSkin::getPageNav()
	 * @see UltimaSelectSkin::getPageContent()
	 */
	public function renderOptions() {
		global $ultima_qodef_Framework;
		$tab         = ultima_qodef_get_admin_tab();
		$active_page = $ultima_qodef_Framework->qodeOptions->getAdminPageFromSlug( $tab );
		if ( $active_page == null ) {
			return;
		}
		?>
	 ...
	 *
	 * @param UltimaQodefAdminPage $page current page to load
	 *
	 * @see QodeSkinAbstract::loadTemplatePart()
	 */
	 ...
	 * @see UltimaSelectSkin::getHeader()
	 * * @see UltimaSelectSkin::getPageNav()
	 * * @see UltimaSelectSkin::getImportContent()
	 */
