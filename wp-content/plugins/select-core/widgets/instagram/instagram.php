...
class UltimaQodefInstagramWidget extends UltimaQodefWidget {
	protected $params;

	public function __construct() {
		parent::__construct(
			'qodef_instagram_widget',
			esc_html__( 'Select Instagram Widget', 'select-core' ),
			array( 'description' => esc_html__( 'Display instagram images', 'select-core' ) )
		);
	...
	protected function setParams() {
		$this->params = array(
			...
			array(
				'name'  => 'tag',
				'type'  => 'textfield',
				'title' => esc_html__( 'Tag', 'select-core' )
			),
			array(
				'name'  => 'number_of_photos',
				'type'  => 'textfield',
				'title' => esc_html__( 'Number of photos', 'select-core' )
			),
			array(
				'name'    => 'number_of_cols',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Number of columns', 'select-core' ),
				'options' => array(
					'2' => 'Two',
					'3' => 'Three',
					'4' => 'Four',
			...
			array(
				'name'    => 'image_size',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Image Size', 'select-core' ),
				'options' => array(
					'thumbnail'           => esc_html__( 'Small', 'select-core' ),
					'low_resolution'      => esc_html__( 'Medium', 'select-core' ),
					'standard_resolution' => esc_html__( 'Large', 'select-core' )
				)
			),
			array(
				'name'  => 'transient_time',
				'type'  => 'textfield',
				'title' => esc_html__( 'Images Cache Time', 'select-core' )
			),
		);
	}
				...
				foreach ( $images_array as $image ) { ?>
                    <li>
                        <a itemprop="url" href="<?php echo esc_url( $instagram_api->getHelper()->getImageLink( $image ) ); ?>" target="_blank">
							<?php echo ultima_qodef_kses_img( $instagram_api->getHelper()->getImageHTML( $image, $image_size ) ); ?>
                        </a>
                    </li>
				<?php } ?>
            </ul>
	...
	public function form( $instance ) {
		...
		if ( ! get_option( 'qode_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				echo '<p><a class="button" href="' . esc_url( $instagram_api->getAuthorizeUrl() ) . '">' . esc_html__( 'Connect with Instagram', 'select-core' ) . '</a></p>';
			}
		}
