 ...
 * @return bool
 */
function ultima_qodef_url_exists( $url ) {
...
if ( ! function_exists( 'qode_core_init_shortcode_loader' ) ) {
	...
	add_action( 'ultima_qodef_shortcode_loader', 'qode_core_init_shortcode_loader' );
}
...
if ( class_exists( 'WP_Block_Type' ) && defined( 'QODE_ROOT' ) ) {
	add_action( 'admin_head', 'ultima_qodef_meta_box_add' );
} else {
	add_action( 'add_meta_boxes', 'ultima_qodef_meta_box_add' );
}

if ( ! function_exists( 'ultima_qodef_create_meta_box_handler' ) ) {
	function ultima_qodef_create_meta_box_handler( $box, $key, $screen ) {
		add_meta_box(
			'qodef-meta-box-' . $key,
			$box->title,
			'ultima_qodef_render_meta_box',
			$screen,
			'advanced',
			'high',
			array( 'box' => $box )
...
if(!function_exists('qode_core_is_rev_slider_installed')) {
	/**
	 * Checks whether Revolution Slider is installed or not
	 * @return bool
	 */
	function qode_core_is_rev_slider_installed() {
