<?php
namespace UltimaQodef\Modules\Shortcodes\Icon;

use UltimaQodef\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package UltimaQodef\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            ...
            'params'                    => array_merge(
                \UltimaQodefIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type'        => 'dropdown',
                    ...
                    array(
                        'type'        => 'dropdown',
                        'heading'     => esc_html__('Flip Icon on Hover', 'select-core'),
                        'param_name'  => 'icon_flip_animation',
                        'admin_label' => true,
                        'value'       => array(
    ...
    public function render($atts, $content = null) {
        ...
        if($params['icon_flip_animation'] == 'yes') {
            $iconHolderClasses[] = 'qodef-flip';
        }
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.ultima_qodef_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.ultima_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.ultima_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.ultima_qodef_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
                ...
                $iconHolderStyles[] = 'border-width: '.ultima_qodef_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.ultima_qodef_filter_px($params['border_radius']).'px';
                }
            }
